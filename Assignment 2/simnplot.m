...
for bloop=1:length(bvec), b=bvec(bloop);
  for i=1:sim
    ...
    b3(i,bloop)=DurbinMA1959(y,1);
  end 
end
%%
figure, set(gca,'fontsize',16)
plot(bvec,mean(b1)-bvec,'r-',bvec,mean(b2)-bvec,'g--',bvec,mean(b3),'b.--','linewidth',3)
title('Bias'), legend('built in function','Marc Paolella code','Durbin1959')
figure, set(gca,'fontsize',16)
plot(bvec, mean((b1-true).^2), 'r-', ...
     bvec, mean((b2-true).^2), 'g--', ...
     bvec, mean((b3-true).^2), 'b.--','linewidth',3)
title('MSE'), legend('built in function','Marc Paolella code','Durbin1959')